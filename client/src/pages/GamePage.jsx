...
const GamePage = () => {
  ...
  useEffect(() => {
    ...
    if (match) {
      ...
      fetch(
        `http://localhost:3000/api/getsinglegame/${gameName}?idGameDb=${idGameDb}&idGameIgdb=${idGameIgdb}`
      )
  ...
  return (
    <main className={"container_main_game_page"}>
      <div className={"container_section_game_page"}>
        <section className={"section_game_page"}>
          ...
          <article className={"container_content_buy_game_page"}>
            {gameData && <h2>{gameData.dataOfGame.name}</h2>}
            <StockComposant />
            {gameData && (
              <span className={"price_content_buy_game_page"}>
                {gameData.dataOfGame.price}€
              </span>
            )}
            <div className={"container_button_buy_game_page"}>
              <a href="#" className={"cart_buy_game_page"}>
                <i className="fa-solid fa-cart-shopping"></i>
              </a>
              <a href="#" className={"buy_game_page"}>
                Buy Now
              </a>
            </div>
          </article>
      ...
      {gameData && (
        <ProductSingleGameDetail
          description={gameData.allDataOfGameByAPI[0].summary}
          idGame={gameData.allDataOfGameByAPI[0].id}
        />
      )}
    </main>
  );
